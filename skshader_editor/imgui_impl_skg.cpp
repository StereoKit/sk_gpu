...
bool ImGui_ImplSkg_Init() {
	...
	void*    pixels[1];
	int      width, height;
	io.Fonts->GetTexDataAsRGBA32((unsigned char **)&pixels[0], &width, &height);

	im_font_tex = skg_tex_create(skg_tex_type_image, skg_use_static, skg_tex_fmt_rgba32_linear, skg_mip_generate);
	skg_tex_set_contents(&im_font_tex, pixels, 1, width, height);
